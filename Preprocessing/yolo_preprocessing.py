...
def label_from_txt(sample_path):
    with open(sample_path, 'r', encoding='utf-8') as f:
            ...
            # ['Pointxy', '0.3170781893004120.479356405585914']
            # ['W', '0.490123456790123']
            for line in lines:
                #공백제거
    ...
    #label_folder 정해놓은 '루트 폴더' 안에 있는 모든 파일
        ...
        for i in range(len(json_list)):
            ...
            with open(json_list[i], 'r', encoding='utf-8') as f:
                ...
                #'경로' 가 없을 때 -> os.mkdir(경로) 만들어줘!
                if not os.path.exists(out_path):
                    os.mkdir(out_path)
    
                #3가지 방법
                txt_path= f'./Data/PeachDataset/peach_label/yolo_txt_label/{filename}.txt'
                txt_path= f'{out_path}/{filename}.txt'
                txt_path= os.path.join(out_path,filename+'.txt')
                # txt_path= txt_path.split('.jpg')
                # print(txt_path)
...
def move_datas(source_folder, destination_folder):
    ...
    for i in os.listdir(source_folder, destination_folder):
        file_list.append(os.path.join(source_folder, i))

    for i in file_list:
        f = i.split('\\')[-1]

        # print(f'{i}에서 {os.path.join(destination, f)}')
        shutil.copy2(i, os.path.join(destination_folder, f))

def move_label_datas(source_dir, train_image_path, valid_image_path, train_target, valid_target):
        #라벨 옮기기
    ...
    # train_image_path = r'./Data/PeachDataset/YoloDataset/images/train'
    # valid_image_path = r'./Data/PeachDataset/YoloDataset/images/valid'

    train_list = [x for x in os.listdir(train_image_path)]
    valid_list = [x for x in os.listdir(valid_image_path)]
    ...
    for target in source_files:
        ...
        for v in valid_list:
            if f in v:
                f = f + '.txt'
                print(target, os.path.join(valid_target, f))    
